...
// import vtkLandmarkTransform from 'vtk.js/Sources/Common/Transform';
...
container.classList.add(styleCSS.container);
...
function initializeDebugHandler(textCanvas) {
  ...
  function drawCircle(x, y) {
    // debugCtx.beginPath();
    // debugCtx.arc(x, y, 20, 0, Math.PI * 2);
    // debugCtx.fill();
    debugCanvas.style.bottom = `${y}px`;
    // debugCanvas.style.right = `${y}px`;
  }
  function draw() {
    // debugCtx.clearRect(0, 0, debugCanvas.width, debugCanvas.height); // 清除画布内容
    if (isDragging) return; // 如果未按下鼠标则不处理移动事件
    drawCircle(curX, curY); // 重新绘制圆圈到新的位置
  }
  textCanvas.addEventListener('mousedown', (e) => {
    isDragging = true;
    startX = e.clientX - curX; // 获取鼠标与圆心的水平距离差值
    startY = e.clientY - curY; // 获取鼠标与圆心的垂直距离差值
  });
  textCanvas.addEventListener('mouseup', (e) => {
    isDragging = false; // 释放鼠标后停止拖动处理
    curX = e.clientX - startX; // 根据鼠标位置和初始位置的差值更新圆的位置
    curY = e.clientY - startY;
    draw(); // 更新画布内容
  });
  debugCanvas.classList.add(styleCSS.debugCanvas, 'debugCanvas');
...
textCanvas.classList.add(styleCSS.container, 'textCanvas');
...
psMapper.setCallback((coordsList, camera, aspect, depthBuffer) => {
  if (textCtx && windowWidth > 0 && windowHeight > 0) {
    ...
    const viewMatrix = camera.getViewMatrix(); // 观察矩阵,从相机坐标系到世界坐标系的转换
  ...
  if (enableDebugCanvas && depthBuffer) {
    if (!debugHandler) {
      debugHandler = initializeDebugHandler(textCanvas);
    }
...
const currentActorPosition = [0, 0, 0]; // 当前 actor 的位置
let pos = ''; // 上次鼠标的位置
let lastPos = ''; // 当前 鼠标的位置
interactor.onMouseWheel(() => {
  // console.log('12');
});
interactor.onLeftButtonPress((event) => {
  console.log('onLeftButtonPress', event);
  pos = event.position; // 我们只关心屏幕坐标，所以 z 是 0
});
interactor.onLeftButtonRelease((event) => {
  console.log('onLeftButtonRelease', event.position, pos);
  lastPos = event.position;
  // 将鼠标位置从视口坐标系转换为世界坐标系
  // const worldPos = renderer.worldPointFromDisplay(lastPos[0], lastPos[1]);
  ...
  if (camera.getParallelProjection()) {
    ...
    // const { center } = model;
    const center = planeActor.getPosition();
    const style = interactor.getInteractorStyle();
    // computeWorldToDisplay 将屏幕或像素空间中的位置转换为世界空间中的位置
    ...
    // planeActor.setPosition(newFp[0], newFp[1], newFp[2]); // 设置 actor 的新位置
  }

  // planeActor.setPosition(
  //   currentActorPosition[0],
  //   currentActorPosition[1],
  //   currentActorPosition[2]
  // ); // 设置 actor 的新位置
  // planeActor.addPosition(1, 0, 0); // 设置 actor 的新位置
  console.log('planeActor', planeActor.getPosition(), currentActorPosition);

  // planeActor.rotation (transform);
  renderWindow.render(); // 重绘场景以显示更改

  // const transform = vtkLandmarkTransform.newInstance();
  // transform.Translate(10, 0, 0); // 平移10个单位
  // transform.RotateZ(45); // 旋转45度
  // planeActor.SetUserTransform(transform);
});
// interactor.initialize();
interactor.start(); // 这将启动渲染循环和鼠标事件监听器
