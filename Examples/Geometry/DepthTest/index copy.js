...
let lastPosition = [0, 0, 0]; // 上次鼠标的位置
const currentActorPosition = [0, 0, 0]; // 当前 actor 的位置

// 当鼠标按下时，记录当前位置并开始拖拽
interactor.handleMouseDown = (event) => {
  lastPosition = [event.position[0], event.position[1], 0]; // 我们只关心屏幕坐标，所以 z 是 0
};

// 当鼠标移动时，更新 actor 的位置
interactor.handleMouseMove = (event) => {
  const dx = event.position[0] - lastPosition[0]; // x 方向上的移动量
  const dy = event.position[1] - lastPosition[1]; // y 方向上的移动量
  currentActorPosition[0] += dx; // 更新 actor 的 x 位置
  currentActorPosition[1] += dy; // 更新 actor 的 y 位置
  currentActorPosition[2] = 0; // 我们只关心屏幕坐标，所以 z 是 0
  lastPosition = event.position; // 更新鼠标的最后位置
  textActor.setPosition(currentActorPosition); // 设置 actor 的新位置
